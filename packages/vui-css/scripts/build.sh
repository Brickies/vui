...
fontname() {
  if command_exists superman ; then
    echo "https://b.yzcdn.cn$server_prefix/$(basename $basepath/../build/font/vui-icon-*.$1)"
  else
    echo "$(abspath $basepath/../build/font/vui-icon-*.$1)"
  fi
}
...
if command_exists superman ; then
  # upload to cdn
  superman cdn $server_prefix $basepath/../build/font/vui-icon-*
fi
...
.v-icon::before {
  font-family: "vui-icon" !important;
  font-style: normal;
  font-weight: normal;
  speak: none;
...
cat $basepath/../build/css/vui-icon-codes.css >> $basepath/../src/icon.css
