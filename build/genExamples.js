...
// var navs = require('../examples/src/nav.config.json');
// navs = navs['zh-CN'];
...
// var docsDir = path.resolve(__dirname, '../examples');
// var components = [];
...
//   var itemMdFile = `${docsDir}/examples${item.path}.md`;
//   if (!fs.existsSync(itemMdFile)) {
//     continue;
//   }

//   var itemMd = fs.readFileSync(`${docsDir}/examples${item.path}.md`).toString();
//   var content = parser.render(itemMd);
//   var result = renderVueTemplate(content, item.title);

//   var exampleVueName = `${docsDir}/pages/${item.path}.vue`;
